...
impl Hand {
    fn new(hand_input: &str) -> Self {
        ...
        let mut hand_type: Vec<_> = hand_matches.into_iter().filter(|&count| count > 0).collect();
    ...
    fn new_with_jokers(hand_input: &str) -> Self {
        ...
        let mut hand_type: Vec<_> = hand_matches[1..].into_iter().copied().filter(|&count| count > 0).collect();
        hand_type.sort_by_key(|&b| cmp::Reverse(b));
...
fn part1(input: &str) -> usize {
    ...
    hands.into_iter().enumerate().map(|(idx, (_hand, bid))| (idx+1) * bid).sum()
}
